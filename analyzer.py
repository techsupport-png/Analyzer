"""
Analyzer - Streamlit app (Gemini) with persistent feedback storage (SQLite)
Features:
- Upload Resume, SOP, LOR (PDF/DOCX/TXT)
- Initial analysis (Resume/SOP/LOR) using Google Gemini (google-generativeai)
- Store 'Areas of Improvement' per user (email) in SQLite
- Re-evaluation: compare previous improvement suggestions with new documents
- Acknowledge changes, suggest further improvements, and give final verdict

How to run:
1. pip install streamlit google-generativeai PyPDF2 python-docx
2. export GOOGLE_API_KEY="your_api_key"
3. streamlit run new_app.py
"""
...
def call_gemini(prompt: str):
    try:
        ...
        # response.text is the simple aggregated string in current SDK
        if hasattr(response, "text") and response.text:
            return response.text
        # fallback: try to pull structured content if present
        try:
            # Some SDK responses may have candidates
...
# -------------------- Streamlit UI --------------------
st.set_page_config(page_title="Analyzer (Email persistence)", layout="wide")
...
if st.button("🚀 Analyze / Re-evaluate"):
    ...
    if not email:
        ...
        # fetch previous feedback for this email+university+program
        ...
        if not any([prev_resume, prev_sop, prev_lor]):
            ...
            # Attempt to parse NEW_OR_REMAINING_ISSUES section and save updated feedback
            new_resume = None
            new_sop = None
            new_lor = None
            ...
            # fallback: if not found, try to locate any lines after 'RESUME:' within that block
...
# ---------- Optional: show previous feedback for convenience ----------
st.markdown("---")
